...
func Analyze(r *model.Report, envOverride string) {
	...
	c := model.Conclusion{
		Environment:     env,
		EnvironmentAuto: auto,
	}

	checkNodes(r, env, &c)
	checkPods(r, env, &c)
	checkWorkloads(r, env, &c)
	checkStorage(r, env, &c)
	checkControlPlane(r, env, &c)
	checkCerts(r, env, &c)
	checkEvents(r, env, &c)
...
func resolveEnv(r *model.Report, override string) (string, bool) {
	if v := strings.ToLower(strings.TrimSpace(override)); v != "" && v != "auto" {
		...
		case "dev", "development":
			return EnvDev, false
		case "stage", "staging":
			return EnvStaging, false
		case "prod", "production":
			return EnvProd, false
		default:
			return v, false
		}
	}
	// 自動推論
	switch r.Cluster.Distribution {
	case "eks", "gke", "aks", "openshift":
		return EnvProd, true
	}
	switch {
	case r.Cluster.NodeCount <= 0:
		return EnvDev, true
	case r.Cluster.NodeCount < 3:
		return EnvDev, true
	case r.Cluster.NodeCount < 10:
...
// 不同環境下的閾值。production 收得最緊，dev 最寬鬆。
...
func thresholdsFor(env string) thresholds {
	switch env {
	case EnvProd:
		return thresholds{cpuWarn: 70, cpuCrit: 85, memWarn: 75, memCrit: 90, certWarnDays: 60, certCritDays: 14, eventsWarn: 10}
	case EnvStaging:
		return thresholds{cpuWarn: 80, cpuCrit: 90, memWarn: 85, memCrit: 95, certWarnDays: 30, certCritDays: 7, eventsWarn: 25}
	default: // dev
		return thresholds{cpuWarn: 90, cpuCrit: 98, memWarn: 90, memCrit: 98, certWarnDays: 14, certCritDays: 3, eventsWarn: 50}
	}
}

func priorityFor(env, severity string) string {
...
func checkNodes(r *model.Report, env string, c *model.Conclusion) {
	if len(r.Nodes) == 0 {
		return
	}
	notReady := 0
	...
	if notReady > 0 {
		...
		addRec(c, env, SeverityCritical, "節點",
	...
	if pressure > 0 {
		...
		addRec(c, env, SeverityWarning, "節點",
			"擴充節點或清理對應壓力來源（磁碟、記憶體、PID、CNI）",
			"長期 Pressure 通常導致 OOMKilled 或 Pod 被驅逐。")
	}
}

func checkPods(r *model.Report, env string, c *model.Conclusion) {
	s := r.PodSummary
	if s.Failed > 0 {
	...
	if s.Unknown > 0 {
		...
		addRec(c, env, SeverityCritical, "Pod",
...
func checkWorkloads(r *model.Report, env string, c *model.Conclusion) {
...
func checkStorage(r *model.Report, env string, c *model.Conclusion) {
	...
	if s.PVCsPending > 0 {
		...
		addRec(c, env, SeverityWarning, "儲存",
...
func checkControlPlane(r *model.Report, env string, c *model.Conclusion) {
	bad := 0
	for _, h := range r.APIHealth {
		if !strings.EqualFold(h.Status, "ok") && !strings.Contains(strings.ToLower(h.Status), "200") {
			bad++
		}
	}
	if bad > 0 {
		...
		addRec(c, env, SeverityCritical, "控制平面",
...
func checkCerts(r *model.Report, env string, c *model.Conclusion) {
	if len(r.Certs) == 0 {
		return
	}
	t := thresholdsFor(env)
	...
	if expired > 0 {
		...
		addRec(c, env, SeverityCritical, "憑證",
	...
	if crit > 0 {
		...
		addFinding(c, SeverityWarning, "憑證",
			...
			fmt.Sprintf("最近一張剩 %d 天。", soonest))
		addRec(c, env, SeverityWarning, "憑證",
			"安排換發時程並建立到期前的告警機制",
			"可結合 cert-manager 或外部監控如 prometheus blackbox。")
	}
}

func checkEvents(r *model.Report, env string, c *model.Conclusion) {
	t := thresholdsFor(env)
	if len(r.Events) > t.eventsWarn {
		addFinding(c, SeverityWarning, "事件",
			fmt.Sprintf("近期警告事件 %d 筆 (門檻 %d)", len(r.Events), t.eventsWarn),
			"事件量過多通常代表有重複失敗的 controller 或 webhook。")
		addRec(c, env, SeverityWarning, "事件",
			"從事件列表找出最頻繁的 reason，集中排除根因",
			"建議將事件導入長期儲存 (Loki / ES) 以便追蹤趨勢。")
	}
}

func checkMetrics(r *model.Report, env string, c *model.Conclusion) {
	if len(r.NodeMetrics) == 0 {
		addFinding(c, SeverityInfo, "監控",
			"metrics-server 未安裝或無法存取",
			"沒有資源使用率資料就難以做容量規劃與 HPA。")
		addRec(c, env, SeverityInfo, "監控",
	...
	if cpuCrit > 0 {
		...
		addRec(c, env, SeverityCritical, "資源",
		...
		addRec(c, env, SeverityWarning, "資源",
...
func checkDistribution(r *model.Report, env string, c *model.Conclusion) {
...
func buildSummary(r *model.Report, c *model.Conclusion) string {
	envSrc := "使用者指定"
	if c.EnvironmentAuto {
		envSrc = "依 cluster 規模與 distribution 自動判定"
	}
...
func addRec(c *model.Conclusion, env, sev, cat, action, rationale string) {
	c.Recommendations = append(c.Recommendations, model.Recommendation{
		Priority:  priorityFor(env, sev),
		Category:  cat,
		Action:    action,
		Rationale: rationale,
	})
