...
// drawHorizBars 在指定區域內畫水平柱狀圖。
//   - x, y: 區塊左上角
//   - w, rowH: 整體寬、單列高
//   - labelW: 左側 label 欄位寬
//   - displayW: 右側 display 欄位寬
//   - barW (= w - labelW - displayW) 為柱子實際寬
func drawHorizBars(pdf *gofpdf.Fpdf, x, y, w, rowH, labelW, displayW float64, bars []HorizBar) {
	...
	for _, b := range bars {
		...
		pdf.Text(x, y+rowH-1.5, truncate(b.Label, 22))
		// 背景軌道
		pdf.SetFillColor(232, 235, 240)
		pdf.Rect(x+labelW, y, barW, rowH-2, "F")
		// 填色塊
		ratio := 0.0
		if b.Max > 0 {
			ratio = b.Value / b.Max
		...
		pdf.Rect(x+labelW, y, barW*ratio, rowH-2, "F")
		// display
		pdf.SetTextColor(40, 40, 40)
		pdf.Text(x+labelW+barW+3, y+rowH-1.5, b.Display)
